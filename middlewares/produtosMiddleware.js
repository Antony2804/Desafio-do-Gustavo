function validarCamposProduto(req, res, next) {
    ...
    if (!nome || !descricao || !preco || !data_atualizado) {
        return res.status(400).json({ error: 'Todos os campos são obrigatórios para criar um produto.' });
    }
    ...
    if (!(data_atualizado instanceof Date) || isNaN(data_atualizado.getTime())) {
        return res.status(400).json({ error: 'A data de atualização do produto é inválida.' });
    }
